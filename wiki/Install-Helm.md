...
> **Note:** `chart.liketrek.com` is a custom domain (CNAME) for the GitHub Pages site at `` — both serve the same chart repository. Using the custom domain keeps your setup working even if the GitHub repository moves again.
...
> ```
>
> Existing releases keep working — only the repo URL changes; future `helm repo update` / `helm upgrade` runs require the new URL. (`` also works — it redirects to `chart.liketrek.com`.)
...
See the [`charts/README.md`](charts/README.md) for all available values.
