...
> **⚠️ Chart repository moved:** The Helm chart is no longer served at `https://mauriceboe.github.io/TREK` (the project moved from a personal repo to the `liketrek` organization). The canonical chart URL is now `https://chart.liketrek.com` — a custom domain (CNAME) for the GitHub Pages site at ``, so it stays stable even if the repository moves again. If your `trek` repo still points to an old URL, switch it before updating:
>
> ```bash
