...
> **Note:** `chart.liketrek.com` is a custom domain (CNAME) for the GitHub Pages site at `` — both URLs serve the same repository. The github.io URL keeps working (it redirects to `chart.liketrek.com`), but the custom domain is the canonical one to use.
