...
/** Outbound GitHub calls: hard timeout and response-size cap. */
...
/** Release checks stay off until GITHUB_RELEASES_REPO is owner/name. */
function githubReleasesRepo(): string | null {
  const repo = process.env.GITHUB_RELEASES_REPO?.trim() ?? '';
...
export class AdminService {
   ...
   * Returns null on any failure; callers decide the fallback.
   */
  ...
  async getGithubReleases(perPage: string = '10', page: string = '1') {
    ...
    const repo = githubReleasesRepo();
    if (!repo) return [];
    const data = await this.fetchGithub(`https://api.github.com/repos/${repo}/releases?${qs}`);
  ...
  async checkVersion(): Promise<VersionInfo> {
    ...
    if (isPrerelease) {
      ...
      const repo = githubReleasesRepo();
      if (!repo) return fail();
      const data = await this.fetchGithub(`https://api.github.com/repos/${repo}/releases?per_page=100`) as
      ...
      const repo = githubReleasesRepo();
      if (!repo) return fail();
      const data = await this.fetchGithub(`https://api.github.com/repos/${repo}/releases/latest`) as
