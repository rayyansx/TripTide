...
/** Remote wiki fallback is off unless GITHUB_WIKI_REPO is owner/name. */
const REPO = /^[\w.-]+\/[\w.-]+$/.test(process.env.GITHUB_WIKI_REPO?.trim() ?? '')
  ? process.env.GITHUB_WIKI_REPO!.trim()
  : '';
const RAW_BASE = REPO ? `https://raw.githubusercontent.com/${REPO}/main/wiki` : '';
...
async function fetchText(file: string): Promise<string> {
  ...
  if (!RAW_BASE) throw new WikiNotFound(file);
...
export async function getWikiAsset(assetPath: string): Promise<{ buf: Buffer; type: string }> {
  ...
  if (!RAW_BASE) throw new WikiNotFound(assetPath);
