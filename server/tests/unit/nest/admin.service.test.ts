...
process.env.GITHUB_RELEASES_REPO = 'example/app';
...
describe('getGithubReleases', () => {
  ...
  it('ADMIN-SVC-053a — clamps the paging query instead of interpolating it raw', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      'https://api.github.com/repos/example/app/releases?per_page=100&page=1',
    );

    await getGithubReleases('10&per_page=999', 'abc');
    expect(fetchMock.mock.calls[1][0]).toBe(
      'https://api.github.com/repos/example/app/releases?per_page=10&page=1',
    );
  });
  ...
  it('ADMIN-SVC-053b — round-trips the paging the admin UI actually sends', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      'https://api.github.com/repos/example/app/releases?per_page=20&page=2',
    );
  });
});
