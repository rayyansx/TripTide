...
Self-hosted travel planner with a React Native app (TripTide) and the API it talks to.

The iOS simulator app is documented in [NATIVE.md](NATIVE.md).
...
See [**How we use AI in TREK**](/discussions/1851) for
the details.
...
See [Install with Docker Compose](wiki/Install-Docker-Compose.md)
...
See [Updating](wiki/Updating.md) — Docker Compose,
...
If you use the MCP addon, the proxy must also pass the `Mcp-Session-Id` header through in both directions on `/mcp` — Nginx and Caddy do this by default, but a proxy that strips it makes every tool call open a new session instead of reusing one. See the [Reverse Proxy wiki page](wiki/Reverse-Proxy.md) for details.
...
[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). See [ATTRIBUTION.md](ATTRIBUTION.md)
